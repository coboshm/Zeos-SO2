...
int sys_fork()
{
  ...
  int end = NUM_PAG_KERNEL + NUM_PAG_CODE + NUM_PAG_DATA + 10;
  for (pag=0;pag<NUM_PAG_DATA;pag++){
    printk(" 1 ");
//    set_ss_pag(tsk->dir_pages_baseAddr, NUM_PAG_KERNEL + NUM_PAG_CODE + pag, new_frames[pag]);
    set_ss_pag(tsk->dir_pages_baseAddr, PAG_LOG_INIT_DATA_P0 + pag, new_frames[pag]);
    printk(" 2 ");
 ...
 //   copy_data((void*)((NUM_PAG_KERNEL + NUM_PAG_CODE + pag)*PAGE_SIZE), (void*)((end + pag)*PAGE_SIZE), PAGE_SIZE);
  ...
  int top = (unsigned long *)tsk->pointer - &tsku->stack[0];
  tsku->stack[top] = &ret_from_fork;
  tsku->stack[top-1] = 0;
  tsk->pointer = &tsku->stack[top-1];
